...
class Policy:
    ...
    Both admissible sets are computed on every step regardless of which one is
    being enforced, so off-support rates are measured against a fixed reference
    and are comparable across mask settings.
    """
    ...
    def record(self, allowed, bare):
        self.n += 1
        for k, m in allowed.items():
            self.size[k] += int(m.sum())
            self.off[k] += int(not bool(m.reshape(-1)[bare]))
    ...
    def choose(self, q, allowed, bare):
        ...
        if not bool(m.any()):
            return bare
        return int(q.masked_fill(~m, -float("inf")).argmax(-1))
    ...
    def act(self, s):
        with torch.no_grad():
            ...
            allowed = self.sets(s)
            bare = int(q.argmax(-1))
            self.record(allowed, bare)
            return self.choose(q, allowed, bare)


class DTPolicy(Policy):
    def __init__(self, net, mu, sd, target, bc, mask, K=20):
        super().__init__(None, bc, mask)
        self.net, self.mu, self.sd, self.target, self.K = net, mu, sd, target, K

    def reset(self):
        self.hist_s, self.hist_a, self.rtg = [], [], self.target

    def act(self, s):
        with torch.no_grad():
            self.hist_s.append(s[0])
            S = torch.stack(self.hist_s[-self.K:])[None]
            a = self.hist_a[-(self.K - 1):] if self.hist_a else []
            A = torch.zeros(1, S.shape[1], dtype=torch.long, device=s.device)
            if a:
                A[0, :len(a)] = torch.tensor(a, device=s.device)
            R = torch.full((1, S.shape[1]), (self.rtg - self.mu) / self.sd,
                           device=s.device)
            q = self.net(R, S, A)[:, -1]
            allowed = self.sets(s)
            bare = int(q.argmax(-1))
            self.record(allowed, bare)
            act = self.choose(q, allowed, bare)
            self.hist_a.append(act)
            return act
...
class RandomPolicy(Policy):
    ...
    def act(self, s):
        with torch.no_grad():
            allowed = self.sets(s)
        bare = int(self.rng.integers(N_ACTIONS))
        self.record(allowed, bare)
        if self.mask == "bare":
            return bare
        ok = torch.nonzero(allowed[self.mask].reshape(-1)).flatten().tolist()
        return int(self.rng.choice(ok)) if ok else bare


# ---------------------------------------------------------------------- evaluation

def evaluate(env, policy, device, n_episodes, base_seed):
    rets = []
    for k in range(n_episodes):
        ...
        while True:
            s = torch.as_tensor(
                encode(obs["image"][None], np.array([obs["direction"]]))
            ).to(device)
            a = policy.act(s)
            obs, r, term, trunc, _ = env.step(a)
            total += float(r)
            if hasattr(policy, "observe_reward"):
                policy.observe_reward(float(r))
            if term or trunc:
                break
        rets.append(total)
        ...
        for seed in range(a.seeds):
            ...
            for arm, make in arms.items():
                ...
                for mask in ("bare", "top3", "bcq_tau"):
                    p = make(mask)
                    mean, sdv = evaluate(env, p, device, a.eval_episodes,
                                         10_000 + 1000 * seed)
                    if mask == "bare":
